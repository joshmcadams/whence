...
| # | Item | Blocks | Effort |
|---|------|--------|--------|
| [01](01-add-remote-and-push.md) ✅ | Create the GitHub repo, add the remote, push | everything | done |
...
| [05](05-review-followups.md) ✅ | Two deferred design calls from the code review | nothing | done |

Suggested order: **04 → 02 → 03** (verify it actually works on the other
OSes before you publish installable artifacts for them).
...
| [imp-06](imp-06-hidden-server-messaging.md) ✅ | Tell the user when servers are hidden instead of "nothing found" | UX | done |
| [imp-07](imp-07-surface-network-exposure.md) ✅ | Surface bind address / network exposure (the data is already collected) | UX | done |
| [imp-08](imp-08-eliminate-redundant-work.md) ✅ | Eliminate redundant work (cache project detection, share snapshot, overlap scan+docker) | perf | done |
| [imp-09](imp-09-polish-grab-bag.md) ✅ | Polish grab-bag (version in `doctor`, JSON uptime, watch flicker, …) | polish | done |

All `imp-*` items are complete. This folder is now historical for items
01/05/imp-01…09; the still-open release-mechanics items are 02–04. Current
and future work tracking lives in `plans/README.md`.
