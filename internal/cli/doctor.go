...
func runDoctor() error {
	...
	if docker.Available() {
		...
		if err != nil {
			row("container runtime", rt+": found, but query failed: "+err.Error())
		} else {
			compose := 0
			...
			row("container runtime", fmt.Sprintf("%s — %d published port(s), %d compose-attributed", rt, len(dockers), compose))
		}
	} else {
