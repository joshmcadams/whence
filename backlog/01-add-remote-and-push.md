...
gh repo create joshmcadams/whence --public --source=. --remote=origin --push
```
...
```sh
cd ~/development/personal/whence
# create an empty repo named "whence" at github.com/joshmcadams first, then:
git remote add origin https://github.com/joshmcadams/whence.git
git push -u origin main
```

## Verify

- `git remote -v` shows `origin` → `github.com/joshmcadams/whence`.
- The **CI** workflow runs on the push and goes green (Actions tab): `go vet`,
  `go test`, the cross-build matrix, and `goreleaser check`.

## Notes

- If you pick a repo name other than `whence`, also update the Go module path in
  `go.mod` and the `-X github.com/joshmcadams/whence/internal/cli.version` ldflags
  path in `.goreleaser.yaml` (search the tree for `joshmcadams/whence`).
